...
The memory address of an array depends on its declaration (global or local). It is always accompanied by a register acting as an index multiplied by the size of its elements. (For example: for an integer array the index is multiplied by 4, expressed as `[ecx * 4]`.) It is important to note that 1 is subtracted from the number of elements because the compiler starts from `0`.
